 ...
 */
export const importFullDatabase = (rawDump: any): { success: boolean; message: string } => {
  if (!rawDump || typeof rawDump !== 'object') {
    throw new Error('Format file backup tidak valid.');
  }

  // Handle both direct dump and wrapped dump (e.g. from Google Drive backup payload)
  const dump = rawDump.data && typeof rawDump.data === 'object' && (rawDump.data.arenas || rawDump.data.allArenasMap || rawDump.data.masterData)
    ? rawDump.data
    : rawDump;

  // Restore Master Data
  const masterData = dump.masterData || rawDump.masterData;
  if (masterData && typeof masterData === 'object') {
    if (masterData.tournament) masterDataState.tournament = masterData.tournament;
    if (Array.isArray(masterData.referees)) masterDataState.referees = masterData.referees;
    if (masterData.assignments) masterDataState.assignments = masterData.assignments;
    if (Array.isArray(masterData.auditLogs)) masterDataState.auditLogs = masterData.auditLogs;
  }

  // Restore Arenas Data (supporting dump.arenas, dump.allArenasMap, or nested data)
  const incomingArenas = dump.arenas || dump.allArenasMap || rawDump.arenas || rawDump.allArenasMap;
  if (incomingArenas && typeof incomingArenas === 'object' && Object.keys(incomingArenas).length > 0) {
    for (const key of Object.keys(arenasMap)) {
      delete arenasMap[key];
    }
    for (const [key, val] of Object.entries(incomingArenas)) {
      arenasMap[key] = val as ArenaContainer;
    }
  } else if (dump.state && dump.tgrState) {
    // Fallback: single arena restore
    const defaultArena = arenasMap['arena_1'] || Object.values(arenasMap)[0];
    if (defaultArena) {
      defaultArena.state = dump.state;
      defaultArena.tgrState = dump.tgrState;
      if (Array.isArray(dump.histories)) defaultArena.histories = dump.histories;
    }
  }

  // Restore Admin Credentials if provided
  const credentials = dump.adminCredentials || rawDump.adminCredentials;
  if (Array.isArray(credentials) && credentials.length > 0) {
    adminCredentials.length = 0;
    credentials.forEach((acc: AdminCredential) => adminCredentials.push(acc));
  }
