...
app.post('/api/action', (req, res) => {
  ...
  try {
    ...
    switch (type) {
      ...
      case 'BULK_DELETE_GELANGGANG': {
        const idsToDelete: string[] = Array.isArray(payload.ids) ? payload.ids : [];
    ...
    const isCriticalAction = [
      'START_MATCH', 'RESET_OR_NEXT_PARTAI', 'SET_BABAK', 'ADD_GELANGGANG', 'DELETE_GELANGGANG', 'BULK_DELETE_GELANGGANG',
...
const setupStaticAndMiddleware = async () => {
  ...
  // Explicit PWA Endpoints with strict compliant headers
  app.get('/sw.js', (req, res) => {
    const swPath = isProduction
      ? path.join(process.cwd(), 'dist', 'sw.js')
      : path.join(process.cwd(), 'public', 'sw.js');
    const fallbackPath = path.join(process.cwd(), 'public', 'sw.js');
    const targetPath = fs.existsSync(swPath) ? swPath : fallbackPath;
  ...
  app.get('/manifest.json', (req, res) => {
    ...
    const targetPath = fs.existsSync(manifestPath) ? manifestPath : fallbackPath;
