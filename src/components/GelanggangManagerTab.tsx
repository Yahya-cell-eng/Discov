...
export default function GelanggangManagerTab({
  ...
  const [selectedArenasForDelete, setSelectedArenasForDelete] = useState<string[]>([]);
  ...
  const openBulkDeleteModal = () => {
    playBeep('click');
    const primaryId = currentArenaId || arenasList[0]?.id;
    // Default candidates: all arenas except the current/primary one
    const candidates = arenasList.filter(a => a.id !== primaryId).map(a => a.id);
    setSelectedArenasForDelete(candidates.length > 0 ? candidates : []);
    setIsBulkDeleteModalOpen(true);
  };

  const toggleSelectArenaForDelete = (id: string) => {
    playBeep('click');
    setSelectedArenasForDelete(prev =>
  ...
  const selectAllExceptPrimary = () => {
    playBeep('click');
    const primaryId = currentArenaId || arenasList[0]?.id;
    const toSelect = arenasList.filter(a => a.id !== primaryId).map(a => a.id);
    setSelectedArenasForDelete(toSelect);
  };

  const selectAllForDelete = () => {
    playBeep('click');
    // Select all except the last one to prevent selecting 100% of arenas
    if (arenasList.length > 1) {
      setSelectedArenasForDelete(arenasList.slice(1).map(a => a.id));
    }
  };

  const unselectAllForDelete = () => {
    playBeep('click');
    setSelectedArenasForDelete([]);
  };

  const handleBulkDeleteSubmit = () => {
    if (selectedArenasForDelete.length === 0) return;

    if (selectedArenasForDelete.length >= arenasList.length) {
      alert('Tidak dapat menghapus semua gelanggang. Minimal harus menyisakan 1 gelanggang aktif.');
      return;
    }

    const arenaNames = arenasList
      .filter(a => selectedArenasForDelete.includes(a.id))
      .map(a => `"${a.nama}"`)
      .join(', ');

    if (window.confirm(`Apakah Anda yakin ingin menghapus ${selectedArenasForDelete.length} gelanggang sekaligus (${arenaNames})? Seluruh data pertandingan di gelanggang terpilih akan dihapus permanen.`)) {
      playBeep('warning');
      dispatch('BULK_DELETE_GELANGGANG', { ids: selectedArenasForDelete });

      if (selectedArenasForDelete.includes(currentArenaId)) {
        const remaining = arenasList.filter(a => !selectedArenasForDelete.includes(a.id));
        if (remaining[0]) {
          onSelectArena(remaining[0].id);
        }
      }
    ...
    <div className="w-full flex flex-col items-center">
      ...
      <div className="w-full max-w-7xl flex flex-wrap items-center justify-between gap-4 p-4 rounded-2xl border border-blue-500/20 bg-[#06081e]/90 backdrop-blur-md shadow-xl mb-4">
        ...
        <div className="flex items-center gap-2.5">
          ...
          {/* Bulk Delete Arenas Button */}
          {arenasList.length > 1 && (
            <motion.button
              whileHover={{ scale: 1.03 }}
              whileTap={{ scale: 0.97 }}
              onClick={openBulkDeleteModal}
              className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-red-950/60 hover:bg-red-900/80 text-red-200 border border-red-800/60 text-xs font-black tracking-wider uppercase shadow-lg shadow-red-950/40 transition-all cursor-pointer"
              title="Hapus Banyak Gelanggang Sekaligus"
            >
              <Trash2 className="w-4 h-4 text-red-400" />
              <span className="hidden sm:inline">HAPUS BANYAK</span>
            </motion.button>
          )}
      ...
      {/* 6. MODAL: HAPUS BANYAK GELANGGANG (BULK DELETE) */}
      <AnimatePresence>
        {isBulkDeleteModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
              className="w-full max-w-xl rounded-2xl border border-red-500/40 bg-[#070d28] p-6 shadow-2xl text-slate-100 max-h-[90vh] flex flex-col"
            >
              {/* Header */}
              <div className="flex items-center justify-between pb-4 border-b border-slate-800">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-base font-black uppercase tracking-wider text-white">
                      HAPUS BANYAK GELANGGANG
                    </h3>
                    <p className="text-xs font-mono text-slate-400">
                      Pilih satu atau beberapa gelanggang untuk dihapus sekaligus.
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 my-4 p-2.5 rounded-xl bg-slate-900/80 border border-slate-800 text-xs">
                <span className="text-slate-400 font-mono">
                  Terpilih: <strong className="text-red-400 font-bold">{selectedArenasForDelete.length}</strong> dari {arenasList.length} gelanggang
                </span>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                  ...
                  <button
                    type="button"
                    onClick={unselectAllForDelete}
                    className="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white text-[11px] font-bold transition-colors cursor-pointer"
                  >
                    Batal Pilih
                  </button>
                </div>
              ...
              {/* Arenas List with Checkboxes */}
              <div className="flex-1 overflow-y-auto space-y-2 pr-1 my-2 max-h-[45vh]">
                {arenasList.map((arena) => {
                  const isChecked = selectedArenasForDelete.includes(arena.id);
                  const isCurrent = currentArenaId === arena.id;
                  const mode = arena.modeAktif || 'tanding';

                  return (
                    <div
                      key={arena.id}
                      onClick={() => toggleSelectArenaForDelete(arena.id)}
                      className={`flex items-center justify-between p-3.5 rounded-xl border transition-all cursor-pointer select-none ${
                        isChecked
                          ? 'bg-red-950/40 border-red-500/70 text-white ring-1 ring-red-500/40'
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="w-5 h-5 rounded bg-blue-500/20 text-blue-400 border border-blue-500/30 font-mono font-black text-[11px] flex items-center justify-center">
                              {arena.kode || '1'}
                            </span>
              ...
              {/* Safety notice */}
              <div className="mt-3 p-3 rounded-xl bg-amber-950/30 border border-amber-800/40 flex items-start gap-2.5 text-xs text-amber-300/90">
                <AlertTriangle className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
                <p className="leading-relaxed">
                  Sistem mewajibkan minimal <strong>1 gelanggang</strong> tetap aktif. Gelanggang yang dihapus akan menghapus riwayat skor dan jadwal di gelanggang tersebut.
                </p>
              </div>
              ...
              <div className="flex items-center justify-between gap-3 pt-4 mt-2 border-t border-slate-800">
                ...
                <button
                  type="button"
                  disabled={selectedArenasForDelete.length === 0 || selectedArenasForDelete.length >= arenasList.length}
                  ...
                  className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer ${
                    selectedArenasForDelete.length === 0 || selectedArenasForDelete.length >= arenasList.length
                ...
                >
                  ...
                  <span>
                    {selectedArenasForDelete.length === 0
                      ? 'Pilih Gelanggang'
                      : selectedArenasForDelete.length >= arenasList.length
                      ? 'Sisakan Minimal 1'
                      : `Hapus ${selectedArenasForDelete.length} Gelanggang`}
                  </span>
                </button>
              </div>
            </motion.div>
