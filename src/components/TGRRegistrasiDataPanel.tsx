...
export default function TGRRegistrasiDataPanel({ theme, state, dispatch, onClose }: TGRRegistrasiDataPanelProps) {
  ...
  // Import CSV / Excel File & Directly Apply to Schedule & Secretary
  const handleImportAthletesCSV = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const processRecords = (parsed: TGRRegistrasiAthlete[]) => {
      if (parsed.length > 0) {
        playBeep('valid');
        const merged = [...athletes, ...parsed];
        saveAthletesLocal(merged);

        // DIRECTLY APPLY to Schedule & Secretary
        const payloadList = merged.map((m, index) => ({
          id: m.id,
          noUrut: index + 1,
          noUndian: m.noUndian || (index + 1),
          partai: `PARTAI ${index + 1}`,
        ...
        saveScheduleLocal(merged.map(a => a.id));
        localStorage.setItem('tgr_peserta_list', JSON.stringify(payloadList));
        localStorage.setItem('silat_tgr_registered_athletes', JSON.stringify(payloadList));

        dispatch('TGR_SET_SISTEM_SENI', { sistem: sistemSeni });
        dispatch('UPDATE_TGR_PESERTA_LIST', { pesertaList: payloadList });
        ...
        showCustomAlert(
          'DATA LANGSUNG DITERAPKAN',
          `Berhasil mengimpor ${parsed.length} atlet seni dan LANGSUNG DITERAPKAN ke Jadwal & Sekretaris Seni!\n\nPartai 1 siap tampil dan dinilai di monitor arena dan panel dewan/juri.`
        );
      } else {
        showCustomAlert('GAGAL IMPORT', 'Tidak ada data atlet seni yang valid di file.');
      }
    };
    ...
    if (isExcel) {
      ...
      reader.onload = (event) => {
        try {
          ...
          const idxNama = headers.findIndex(h => h.includes('nama') || h.includes('atlet') || h.includes('peserta'));
          const idxKontingen = headers.findIndex(h => h.includes('kontingen') || h.includes('perguruan') || h.includes('tim') || h.includes('daerah'));
          const idxKategori = headers.findIndex(h => h.includes('kategori') || h.includes('kelas') || h.includes('jurus'));
          const idxUsia = headers.findIndex(h => h.includes('usia') || h.includes('golongan') || h.includes('tingkat'));
          const idxGender = headers.findIndex(h => h.includes('gender') || h.includes('kelamin') || h.includes('jk'));
          const idxPool = headers.findIndex(h => h.includes('pool'));
          const idxUndian = headers.findIndex(h => h.includes('undian') || h.includes('urut'));

          const parsed: TGRRegistrasiAthlete[] = [];
          for (let i = 1; i < rows.length; i++) {
            const row = rows[i];
            if (!row || row.length === 0) continue;
            const nama = String(idxNama !== -1 ? row[idxNama] : row[1] || row[0] || '').trim();
            const kontingen = String(idxKontingen !== -1 ? row[idxKontingen] : row[2] || row[1] || '').trim();
            if (!nama || nama.toLowerCase().includes('nama')) continue;

            const kelas = String(idxKategori !== -1 && row[idxKategori] ? row[idxKategori] : TGR_CATEGORIES[0]).trim();
            const usia = String(idxUsia !== -1 && row[idxUsia] ? row[idxUsia] : AGE_CATEGORIES[0]).trim();
            const genderRaw = String(idxGender !== -1 && row[idxGender] ? row[idxGender] : 'Putra').trim();
            const gender = genderRaw.toLowerCase().includes('putri') || genderRaw.toLowerCase().includes('pi') ? 'Putri' : 'Putra';
            const pool = String(idxPool !== -1 && row[idxPool] ? row[idxPool] : 'Pool A').trim();
            const noUndian = parseInt(String(idxUndian !== -1 ? row[idxUndian] : i), 10) || i;

            parsed.push({
              id: `tgr_xls_${Date.now()}_${i}_${Math.random().toString(36).substr(2, 4)}`,
              nama,
          ...
          showCustomAlert('GAGAL IMPORT EXCEL', `Format file Excel tidak valid: ${err?.message}`);
        }
      };
      ...
      reader.onload = (event) => {
        try {
          ...
          showCustomAlert('GAGAL IMPORT CSV', `Format file tidak valid: ${err.message}`);
        }
      };
              ...
              <div className="lg:col-span-4 flex flex-col gap-4">
                ...
                <div className={`p-4 rounded-2xl border ${
                  ...
                  <label className="mt-2 w-full p-2 bg-slate-900 hover:bg-slate-850 border border-slate-800 text-slate-300 text-[10px] font-bold rounded-lg cursor-pointer flex items-center justify-center gap-1 block text-center">
                    ...
                    <span>Upload Excel / CSV Atlet</span>
                    <input type="file" accept=".csv, .xlsx, .xls" onChange={handleImportAthletesCSV} className="hidden" />
                  </label>
