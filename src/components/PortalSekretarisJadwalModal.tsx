...
16. SITI RAHMA - JAWA TENGAH - Seni Tunggal Putri Dewasa`;
  ...
  const handleApplySmartParsedData = async () => {
    ...
    // Directly apply Tanding categories and load Match 1 to Sekretaris Tanding
    if (dispatch && tandingCategories.length > 0) {
      dispatch('UPDATE_BAGAN_CATEGORIES', { categories: tandingCategories });

      const allMatches = tandingCategories.flatMap(c => c.matches.map(m => ({ cat: c, match: m })));
    ...
    // Also directly apply Seni to schedule and Sekretaris Seni
    ...
    const appliedInfo = [];
    if (tandingCategories.length > 0) appliedInfo.push(`${tandingCategories.length} bagan tanding (Partai 1 siap di Sekretaris Tanding)`);
    if (seniPesertaList.length > 0) appliedInfo.push(`${seniPesertaList.length} peserta seni (Partai 1 siap di Sekretaris Seni)`);

    alert(`Data ${parsedPreviewList.length} peserta berhasil di-upload dan LANGSUNG DITERAPKAN pada Jadwal dan Sekretaris masing-masing kategori!\n\n• ${appliedInfo.join('\n• ')}`);
    setActiveTab('all');
  };
