...
export default function RegistrasiDataPanel({ theme, state, dispatch, onClose }: RegistrasiDataPanelProps) {
  ...
  // Persist and broadcast categories update, and directly apply the first match to the live schedule and secretary
  const updateCategories = (newCategories: BaganCategory[]) => {
    ...
    localStorage.setItem('silat_bagan_categories', JSON.stringify(newCategories));
    dispatch('UPDATE_BAGAN_CATEGORIES', { categories: newCategories });

    // Directly apply first match to Sekretaris Tanding & Live Arena so it is immediately primed
    if (newCategories.length > 0) {
      const allMatches = newCategories.flatMap(c => c.matches.map(m => ({ cat: c, match: m })));
      const firstPlayableMatch = allMatches.find(item => 
        item.match.partai && (item.match.partai.toLowerCase().includes('partai 1') || item.match.partai === 'Partai 1')
      ) || allMatches.find(item => 
        item.match.atletMerah?.nama && item.match.atletBiru?.nama && 
        item.match.atletMerah.nama !== 'BYE' && item.match.atletBiru.nama !== 'BYE'
      ) || allMatches[0];

      if (firstPlayableMatch && (state.matchStatus === 'idle' || state.matchStatus === 'selesai' || !state.matchStatus)) {
        const { cat, match: m1 } = firstPlayableMatch;
        const cleanKelas = cat.name.replace(/\s*\(.*\)/, '');
        dispatch('LOAD_BAGAN_MATCH', {
          namaEvent: state.namaEvent || "Kejuaraan Pencak Silat",
          partai: m1.partai ? m1.partai.replace(/Partai\s+/i, '') : '1',
          kelas: cleanKelas,
          gender: cat.gender,
          selectedWaktu: state.selectedWaktu || 120,
          activeBaganCategoryId: cat.id,
          activeBaganMatchId: m1.id,
          atletMerah: {
            nama: m1.atletMerah?.nama || "Sudut Merah",
            kontingen: m1.atletMerah?.kontingen || "SUDUT MERAH"
          },
          atletBiru: {
            nama: m1.atletBiru?.nama || "Sudut Biru",
            kontingen: m1.atletBiru?.kontingen || "SUDUT BIRU"
          }
        });
      }
    }
  ...
  const handleApplySmartAthletes = async () => {
    if (smartParsedList.length === 0) return;
    playBeep('valid');

    // Auto group both Tanding and Seni records intelligently
    ...
    const tandingAthletes = smartParsedList.filter(p => p.kategoriType === 'Tanding');
    const newAthletes: RegistrasiAthlete[] = (tandingAthletes.length > 0 ? tandingAthletes : smartParsedList).map((item, idx) => ({
    ...
    // Apply Tanding brackets and schedule
    if (tandingCategories.length > 0) {
      updateCategories(tandingCategories);
      setSelectedCatId(tandingCategories[0].name);
    } else {
      rebuildBracketsFromAthletes(merged);
    }

    // Directly apply Seni to schedule and secretary if present
    if (seniPesertaList.length > 0) {
      localStorage.setItem('tgr_peserta_list', JSON.stringify(seniPesertaList));
      localStorage.setItem('silat_tgr_registered_athletes', JSON.stringify(seniPesertaList));
    ...
    const info = [];
    if (tandingCategories.length > 0) info.push(`${tandingCategories.length} bagan tanding (Partai 1 siap di Sekretaris Tanding)`);
    if (seniPesertaList.length > 0) info.push(`${seniPesertaList.length} peserta seni (Partai 1 siap di Sekretaris Seni)`);

    alert(`Data atlet berhasil di-upload dan LANGSUNG DITERAPKAN pada Jadwal dan Sekretaris masing-masing kategori!\n\n• ${info.join('\n• ')}`);
  };
